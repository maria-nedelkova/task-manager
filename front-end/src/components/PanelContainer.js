...
export function PanelContainer({ lists, panelToHighlight, onAdd }) {
  ...
  const calculateContainerHeight = () => {
    const { width } = document.getElementsByClassName('panel-container')[0].getBoundingClientRect();
  ...
  const renderPanels = () => {
    const panels = lists.map((list, index) => {
      const highlight = list.id == panelToHighlight;
      return (
        <Panel
          key={list.id}
          id={list.id}
